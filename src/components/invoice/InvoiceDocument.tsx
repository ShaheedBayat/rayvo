          ...
          <div>
            ...
            {company && (
              <div className="text-sm text-muted-foreground space-y-0.5">
                ...
                {company.taxNumber && (
                  <p className="mono text-xs mt-2">Tax/VAT: {company.taxNumber}</p>
                )}
              </div>
            )}
          </div>
        ...
        <table className="w-full text-sm mb-8">
          <thead>
            <tr className="border-b-2 border-border">
              ...
              <th className="py-3 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground w-16">
                Qty
              </th>
              <th className="py-3 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground w-28">
                Price
              </th>
              <th className="py-3 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground w-28">
                Amount
          ...
          <tbody>
            {invoice.items.map((item) => (
              <tr key={item.id} className="border-b border-border/50">
                ...
                <td className="py-3 text-right mono text-muted-foreground">
                  {formatCurrency(item.unitPrice, invoice.currency)}
                </td>
                <td className="py-3 text-right mono font-medium text-foreground">
        ...
        <div className="flex justify-end">
          <div className="w-72 space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="mono">
                {formatCurrency(calculateTax(invoice.items, invoice.taxRate), invoice.currency)}
              </span>
            </div>
            <div className="flex justify-between text-lg font-semibold border-t-2 border-border pt-3 mt-1">
              ...
              <span className="mono text-primary">
                {formatCurrency(
                  calculateTotal(invoice.items, invoice.taxRate),
                  invoice.currency
                )}
              </span>
            </div>
